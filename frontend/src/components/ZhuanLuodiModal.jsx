...
import { Modal, Form, Row, Col, Input, InputNumber, Select, DatePicker, Radio, Descriptions, message } from 'antd'
...
/**
 * 签约转落地弹窗：签约阶段信息只读回显，补充落地阶段字段后推进至落地。
 * 落地为最终阶段、不涉及审核，无需审核状态字段。
 */
export default function ZhuanLuodiModal({ open, projectData, onCancel, onOk }) {
  ...
  useEffect(() => {
    if (open && projectData) {
      ...
      form.setFieldsValue({
        landingDate: toDayjs(g('landingDate', '落地时间')) || dayjs(),
  ...
  return (
    ...
    >
      ...
      >
        <Descriptions.Item label="项目名称" span={2}>{clean(projectData?.['项目名称']) || '-'}</Descriptions.Item>
        <Descriptions.Item label="编号">{clean(projectData?.['编号']) || '-'}</Descriptions.Item>
        <Descriptions.Item label="项目状态">{clean(projectData?.['项目状态']) || '-'}</Descriptions.Item>
        <Descriptions.Item label="投资金额(亿元)">{clean(projectData?.['投资金额(亿元)']) ?? '-'}</Descriptions.Item>
        <Descriptions.Item label="固投金额(亿元)">{clean(projectData?.['固投金额(亿元)']) ?? '-'}</Descriptions.Item>
        <Descriptions.Item label="协议签订时间">{clean(projectData?.['协议签订时间']) || '-'}</Descriptions.Item>
        <Descriptions.Item label="申报人">{clean(projectData?.['申报人']) || '-'}</Descriptions.Item>
      </Descriptions>
